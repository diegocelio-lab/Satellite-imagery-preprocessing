...
template <typename T>
void input_rgb(const T* input, uint16_t* R, uint16_t* G, uint16_t* B, size_t size)
{
    static_assert(std::is_same<T, uint8_t>::value || std::is_same<T, uint16_t>::value,
                  "T must be uint8_t or uint16_t");

    for (size_t n = 0; n < size; n++) {
        R[n] = static_cast<uint16_t>(input[n]);
        G[n] = static_cast<uint16_t>(input[size + n]);
        B[n] = static_cast<uint16_t>(input[2*size + n]);
    }
}

/* --- Convert separate RGB doubles to interleaved output (8-bit or 16-bit) --- */
template <typename T>
void rgb_output(const uint16_t* Rout, const uint16_t* Gout, const uint16_t* Bout, T* output, size_t size)
{
    static_assert(std::is_same<T, uint8_t>::value || std::is_same<T, uint16_t>::value,
                  "T must be uint8_t or uint16_t");
    ...
    for (size_t n = 0; n < size; n++) {
        if constexpr (std::is_same<T, uint16_t>::value) {
            output[n] = std::clamp<uint16_t>(Rout[n], 0, 65535);
            output[size + n] = std::clamp<uint16_t>(Gout[n], 0, 65535);
            output[2*size + n] = std::clamp<uint16_t>(Bout[n], 0, 65535);
        } else {
            output[n] = std::clamp<uint16_t>(Rout[n], 0, 255);
            output[size + n] = std::clamp<uint16_t>(Gout[n], 0, 255);
            output[2*size + n] = std::clamp<uint16_t>(Bout[n], 0, 255);
        }
    }
}
