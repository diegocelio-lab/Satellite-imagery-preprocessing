 ...
 */
int Launch(
    const std::vector<std::vector<float>>  &imIn, 
    const std::vector<ImageSize>           &imSize,
    const size_t                            histogram_size,
    const float                             bitdepth,
    const bool                              verbose,
    std::vector<std::vector<float>>         *imOut,
    const std::vector<std::vector<bool>>   &img_nodata
    );
...
int ComputeMidwayEqualization(
    std::vector<const float*>              &dataIn_ptr,
    const std::vector<ImageSize>                &imSize,
    const float                             minimum_value,
    const float                             maximum_value,
    const size_t                            histogram_size,
    std::vector<float*>                    &dataOut1_ptr,
    const std::vector<std::vector<bool>>   &img_nodata
    );
 ...
 */
std::vector<std::vector<size_t>> ComputeContrastChangeFunction(
