    ...
    )
{
  ...
  if(loadImage(filename_image_png, image, imSize, false, isTIF, &nodata_values) != EXIT_SUCCESS) {
    return EXIT_FAILURE;
  }
  ...
  if(cumulative)
  {
    ...
    for (size_t channel = 0; channel < imSize.nChannels; ++channel)
    {
      ...
      std::vector<bool> v(imSize.wh, true);

      vector<float>  channel_histogram = ComputeNormalizedCumulativeHistogram(data_ptr, imSize.wh, 0, 255, 256, v);
